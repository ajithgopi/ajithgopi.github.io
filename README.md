...
- **Optional real LLM engines** (engine menu in the chat header), both grounded with the same retrieval as RAG context:
  - *In-browser WebGPU* via [WebLLM](https://github.com/mlc-ai/web-llm) — the model downloads as soon as you pick the engine (progress card in the chat, chat paused until it's ready, one-click fallback to the built-in engine), then is cached by the browser.
